...
module.exports = function(msg) {
	...
	if (!msg.attachments.array()[0] && !is_url) {
		msg.channel.send("You need to attach an image. **"+global.prefix+"hah (url or image)**");
		return;
	} 
	...
	request({url: file_url, encoding: "binary"}, function(err, res, body) {
		...
		fs.writeFile(file_dir, body, "binary", function(err) {
			...
			child("convert", [file_dir, "-gravity", "West", "-chop", "50%x0", file_dir+"_r."+filetype], (close) => {
				if (close != 0) {
				...
				child("convert", [file_dir+"_r."+filetype, "-flop", file_dir+"_l."+filetype], (close) => {
					if (close != 0) {
						msg.channel.send("Error whilst converting image!");
						msg.channel.stopTyping();
						global.log(err);
					...
					child("montage", [file_dir+"_l."+filetype, file_dir+"_r."+filetype, 
						"-geometry", "+0+0", "-background", "none", file_dir+"_hah."+filetype], (close) => {
						if (close != 0) {
							msg.channel.send("Error whilst converting image!");
							msg.channel.stopTyping();
							global.log(err);
						...
						msg.channel.send(new Discord.Attachment(file_dir+"_hah."+filetype));
						msg.channel.stopTyping();

						global.db.exec("UPDATE stats SET value = value + 1 WHERE key = 'hah';");
					}); // holy
				}); // fucking
			}); // shit
